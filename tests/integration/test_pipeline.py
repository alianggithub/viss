...
class FakeOcr:
    def recognize_at(self, media_path, timestamps):
        return [
            EvidenceEvent(
                f"o{i}", stamp, None, "ocr", "recognized_text", 0.9, "fake-ocr",
                {"text": f"Title {i + 1}"},
            )
...
def test_pipeline_produces_valid_deliverables(tmp_path: Path) -> None:
    ...
    config.vision_recognition.mode = "off"
    ...
    frame_index = read_json(run / "frames/index.json")
    assert frame_index["output_mode"] == "annotated_only"
    assert all(row["frame_path"].startswith("frames/segment-") for row in frame_index["frames"])
    ...
    for relative in (
        "config.effective.yaml", "segments.raw.json", "transcript/transcript.md",
        "evidence/audio-events.json", "evidence/visual-events.json",
        "evidence/ocr-events.json", "evidence/boundary-candidates.json", "logs/run.jsonl",
    ):
    ...
    assert "Reviewed Beta" in (run / "frames/index.md").read_text(encoding="utf-8")
    assert len(list((run / "frames").glob("*.jpg"))) == 3
    assert not (run / "frames/annotated").exists()

    analyze(source, run, config, resume=True, transcriber=FakeTranscriber(), ocr_provider=FakeOcr())
    assert "Reviewed Beta" in (run / "chapters.md").read_text(encoding="utf-8")
    assert validate_run(run) == []
