...
def scene_slug(title: str, fallback: str, max_chars: int = 64) -> str:
    ...
    # Reserve room below common 255-byte filename limits for the index,
    # timestamp, separators, and extension. CJK characters use multiple bytes.
...
def render_frame_annotations(
    ...
    for index, segment in enumerate(_segment_dicts(segments), start=1):
        ...
        slug = scene_slug(
            ...
            str(segment.get("id") or f"scene-{index:04d}"),
            config.max_scene_filename_chars,
        )
        filename = (
            f"{index:04d}__{filename_timestamp(timestamp_s)}__{slug}.jpg"
            if config.scene_aware_filenames
    ...
    atomic_write_json(
        ...
        {"schema_version": "1.0", "frame_count": len(rows), "frames": rows},
    )
    fields = list(rows[0]) if rows else [
        ...
        "original_path", "annotated_path", "annotated_filename", "quality_score",
